...
--Script information
local name = "FlyLog"
local LuaVersion = "v0.1"

--Variable
local CrsfField = { "RxBt", "Curr", "Ptch", "Capa", "Bat%", "Roll", "Yaw", "1RSS", "2RSS", "RQly", "FM" }
...
--Define
local TeleItems = 11
local TablePtch = { 3, 10 }
local TableRoll = { 6, 100 }
local TableYaw = { 7, 100 }
local FmIndex = 11
...
local SetColorFlag = BLACK
local BatteryOnFlag = false
local InitSyncFlag = false
local SpoolUpFlag = false
local DisplayLogFlag = false
local WriteEnFlag = false
local SlidingFlag = false
local RingStartFlag = false
local RingEndFlag = false

local options = {
    { "TelemetryValueColor", COLOR,  BLACK },
    { "ProtocolType",        VALUE,  0,    0, 1 },
...
local function create(zone, options)
    ...
    --Protocol Type
    ProtocolType = widget.options.ProtocolType
    ...
    --Parse the data
    StrTemp = string.sub(LogInfo, 21, 23)
...
local function update(widget, options)
    ...
    print("ThrottleChannel = " .. widget.options.ThrottleChannel)
end
...
local function FuelPercentage(x, y, capa, number)
    ...
    lcd.drawText(x, y - 10, string.format("%d%%", number), CENTER + VCENTER + DBLSIZE + SetColorFlag)
...
local function refresh(widget, event, touchState)
    ...
    for k = 1, TeleItems - 1 do
        ...
        if TableID[k][2] then
            ...
            if ProtocolType == 0 then
                if k == TablePtch[1] then
                    GetValue = GetValue * TablePtch[2];
                elseif k == TableRoll[1] then
                    GetValue = GetValue * TableRoll[2];
                elseif k == TableYaw[1] then
                    GetValue = GetValue * TableYaw[2];
                end
            end
    ...
    if WidgetFlag then --Widget 392x168
        ...
        --Number of flights
        ys = ys + yLineHt - 18
        lcd.drawText(xs + 10, ys, string.format("%02d", FlyNumber), XXLSIZE + SetColorFlag)
